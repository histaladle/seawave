...
uniform sampler2D qt_DepthTextures[MAX_LIGHT];
...
uniform ShadowProperty qt_Shadows[MAX_LIGHT];
...
void main(void)
{
    vec4 pos=/*qt_View**/qt_Model*vPos;
    vec4 texColor;
    texColor=vColor;
    ...
    for(int i=0;i<MAX_LIGHT;i++)
    {
        ...
        //[start] shadow parameters
        float visibility=1.0;
        if(qt_Shadows[i].isEnable) {
            vec4 projected_pos=qt_Shadows[i].projection*qt_Shadows[i].view*pos;
            vec4 depth_color=texture2DProj(qt_DepthTextures[i],projected_pos);
            if(depth_color.x<projected_pos.z/projected_pos.w) {
                visibility=0.2;
            }
        }
        //[end] shadow parameters
        vec3 rgb=visibility*min(qt_Material.emission
